...
<body>
    ...
    <div class="print:hidden no-print fixed top-6 right-6 z-50 flex gap-4">
        ...
        <form action="{{ ($isProforma ?? false) ? route('admin.events.proforma-email', $event) : route('admin.events.email-invoice', $event) }}" method="POST" class="flex items-center">
            @csrf
            <div class="relative flex items-center">
                ...
                <input type="email" name="email" value="{{ $event->customer_email }}" class="pl-10 pr-4 py-2 bg-white border border-gray-200 rounded-l-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none w-72 shadow-inner transition-all" placeholder="Recipient Email">
            </div>
