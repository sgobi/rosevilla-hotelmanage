...
<body>
    ...
    <div class="print:hidden no-print fixed top-6 right-6 z-50 flex gap-4">
        ...
        <form action="{{ ($isProforma ?? false) ? route('admin.events.proforma-email', $event) : route('admin.events.email-invoice', $event) }}" method="POST" class="flex items-center gap-2">
            @csrf
            <div class="relative group">
                <input type="email" name="email" value="{{ $event->customer_email }}" class="bg-white text-gray-900 border border-gray-200 px-4 py-2 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none w-64 shadow-lg transition-all" placeholder="Recipient Email">
